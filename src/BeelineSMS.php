<?php
namespace Amorpha\BeelineSms;

class BeelineSMS {

	public $user = '';		// ваш логин в системе
	public $pass = '';		// ваш пароль в системе
	public $hostname = 'beeline.amega-inform.ru'; 			// host замените на адрес сервера указанный в меню "Поддержка -> протокол HTTP"
	public $path = '/sms_send/';
	public $proxy_data = false;
	public $post_data = [];			// данные передаваемые на сервер
	public $multipost = false;			// множественный запрос по умолчанию false
    ...
    public function status_sms($date_from, $date_to, $smstype, $sms_group_id, $sms_id) {
		...
		$in = ['action' => ($smstype == "RECVSMS" ? "inbox" : "status")];
